...
if [[ "$ADB_OCID" != "MISSING" ]]; then
  ...
  if [[ "$ACL_CHECK" == *"192.168.0.64"* ]]; then
    check 4 "ADB-S ACL includes OAC PAC subnet (192.168.0.64/26)" "192.168.0.64" "$ACL_CHECK"
  else
    echo -e "  [${FAIL}] #4: ADB-S ACL does NOT include 192.168.0.64/26 — got: ${ACL_CHECK}"
    ((CHECKS_FAILED++))
  fi
fi
...
if [[ "$ADB_PE_IP" == 192.168.0.* ]]; then
  # ADB-S subnet is 192.168.0.0/26 (IPs .1 through .62)
  check 6 "ADB-S private IP in adb subnet (192.168.0.0/26)" "192.168.0." "$ADB_PE_IP"
else
  echo -e "  [${FAIL}] #6: ADB-S private IP '${ADB_PE_IP}' not in 192.168.0.0/26"
  ((CHECKS_FAILED++))
fi

# ─── Check 7: PAC egress IP in correct subnet ────────────────────────────────

if [[ "$PAC_IP" == 192.168.0.* ]]; then
  # PAC subnet is 192.168.0.64/26 (IPs .65 through .126)
  check 7 "PAC egress IP in oac-pac-sub (192.168.0.64/26)" "192.168.0." "$PAC_IP"
else
  echo -e "  [${FAIL}] #7: PAC IP '${PAC_IP}' not in 192.168.0.64/26"
  ((CHECKS_FAILED++))
fi
